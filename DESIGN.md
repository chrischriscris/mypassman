...
- **Clipboard**: advisory concealment only (`org.nspasteboard.ConcealedType`
  ...
  clipboard wherever possible. *Implemented in CLI: JXA/NSPasteboard write
  with Concealed+AutoGenerated markers, detached `__clipclear` janitor
  clears after 45 s iff unchanged (payload hash via stdin, never argv).*
